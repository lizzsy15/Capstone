...
html="""<!-- Log in box-->
      ...
      <div class="infoBlock">
        <form method="post" action="ForbiddenWordsAdd.cgi" class="infoBlock">
          ...
          {content}        </form><div class="pageRow">{PageContent}</div>
      </div>
		</article>"""
...
if admin == True:
        if userID != "":
                try:
                        SQL = "SELECT count(userID) from user where type = 'administrator';"
                        cursor.execute(SQL)
                        ...
                        if lastPage == False:
                                ...
                                for i in range(4):
                                        if ((int(page_count)+1)*5)>int(totalNum):
                                                PageContent +='<a href="adminList.cgi?page='+str(page_count)+'">'+str(page_count)+'</a>'
                                                break
                                        else:
                                                PageContent +='<a href="adminList.cgi?page='+str(page_count)+'">'+str(page_count)+'</a>'
                                                page_count += 1
                                PageContent +='<a href="adminList.cgi?page='+str((int(page)+1))+'"><i class="fas fa-arrow-right"></i></a>'
                        else:
                                PageContent +='<a href="#"><i class="fas fa-arrow-left"></i></a>'
                                PageContent +='<a href="#" class="currentPage">1</a>'
                ...
                if page != 1:
                        if lastPage == False:
                                PageContent +='<a href="adminList.cgi?page='+str((int(page)-1))+'"><i class="fas fa-arrow-left"></i></a>'
                                PageContent +='<a href="adminList.cgi?page='+str((int(page)-1))+'">'+str(int(page)-1)+'</a>'
                                page_count = int(page)
                                for i in range(4):
                                        if ((int(page_count))*5)>int(totalNum):
                                                PageContent +='<a href="adminList.cgi?page='+str(page_count)+'" class="currentPage">'+str(page_count)+'</a>'
                                                break
                                        else:
                                                PageContent +='<a href="adminList.cgi?page='+str(page_count)+'">'+str(page_count)+'</a>'
                                                page_count += 1
                ...
                page = int(page)
                try:
                        SQL = "select icon, username, userID from user where type = 'administrator';"
                        cursor.execute(SQL)
                        results = cursor.fetchall()
                except Exception as e:
                        print('<p>Something went wrong with the SQL!</p>')
                        ...
                        for row in results:
                                if post_count <= int(page)*5 and post_count >(int(page)-1)*5:
                                        if str(row[0]) != "None":
                                                content+='<p><img src="images/user/'+str(row[0])+'" alt="logo">'
                                        else:
                                                content+='<p><img src="images/user/nullicon.jpg" alt="logo">'
                                        content+='</img><a href="userProfile.cgi?userid='+str(row[2])+'">'+row[1]+'</a>'
                                        content+='<a href="adminRemove.cgi?userid='+str(row[2])+'">Remove</a></p>'
                                        post_count += 1
                                else:
                                        post_count += 1
